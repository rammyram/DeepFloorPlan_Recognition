...
def train(nn_model,train_set_loader,val_set_loader,loss_func,optimizer, config):
    ...
    for epoch in range(config.epochs):
        ...
        for batch_id,(image,gt) in enumerate(train_set_loader):
            ...
            nn_model.train()
            if(configuration.training_config.device.type == 'cuda'):
                image,gt = image.type(torch.cuda.FloatTensor).to(configuration.training_config.device.type),gt.type(torch.cuda.LongTensor).to(configuration.training_config.device.type)
            else:
                image,gt = image, gt
